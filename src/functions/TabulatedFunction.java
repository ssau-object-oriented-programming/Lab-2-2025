...
public class TabulatedFunction {
    private static final double EPSILON = 1e-9; // Tolerance for comparing x coordinates.
    ...
    public void addPoint(FunctionPoint point) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (Math.abs(point.getX() - pointX) <= EPSILON) {
                throw new IllegalArgumentException("Point with the same x already exists.");
            }
    ...
    private void ensureCorrectOrder(double x, int index) {
        if (index > 0) {
            ...
            if (x <= previousX) {
                throw new IllegalArgumentException("Points must be in ascending order by x.");
            }
        }
        if (index < size - 1) {
