...
public class TabulatedFunction {
    ...
    public TabulatedFunction(double leftX, double rightX, int pointsCount) {
        ...
        double intervlen= Math.abs((rightX-leftX))/(pointsCount-1);

        for(int i = 0; i < pointsCount; i++){
            points_arr[i] = new FunctionPoint(leftX+intervlen*i, 0);
        }
    ...
    public TabulatedFunction(double leftX, double rightX, double[] points) {
        ...
        double intervlen= Math.abs((rightX-leftX))/(points.length-1);
        if (pointslength != 0) {
            for (int i = 0; i < points.length; i++) {
                points_arr[i] = new FunctionPoint(leftX+intervlen*i / (pointslength - 1), points[i]);
            }
        }
    }
