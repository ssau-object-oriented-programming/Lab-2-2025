...
public class TabulatedFunction {
    ...
    public TabulatedFunction(double leftX, double rightX, int pointsCount) {
        ...
        if (pointsCount > 1) { //проверка на то, чтобы точек было больше одной
    ...
    public void setPoint(int index, FunctionPoint point) { //замена точки
        double leftPoint;
        double rightPoint;
        ...
        if (leftPoint <= point.getx() && rightPoint >= point.getx()) { //если точка в интервале, меняем ее значение
    ...
    public void setPointX(int index, double x) {  //изменение x у точки с заданным индексом
    ...
    public void deletePoint(int index) {
        if (index >= 0 && index < arrayLen) { //если индекс принадлежит интервалу
